...
export interface Visitor<T> {
  ...
  visitReceiveVarStmt(stmt: ReceiveVarStmt): T;
  ...
  visitReceiveStmt(stmt: ReceiveStmt): T;
...
export class ClassStmt extends Stmt {
  ...
  constructor(
    name: Token,
    superclass: IdentifierTypeExpr | undefined,
    fields: Parameter[],
    initializer: ProcedureStmt | undefined,
    methods: (FunctionStmt | ProcedureStmt)[],
    overrides: Map<ProcedureStmt | FunctionStmt, true>
  ) {
    super();
    this.name = name;
...
export class ReceiveVarStmt extends Stmt {
  readonly name: Token | GetExpr;
  readonly type: TypeExpr | undefined;
  ...
  accept<T>(visitor: Visitor<T>): T {
    return visitor.visitReceiveVarStmt(this);
  }
}
...
export class ReceiveStmt extends Stmt {
  readonly keyword: Token;
  readonly object: Expr;
  readonly sender: Token | Expr;
